...
| 模块 | 职责 |
|------|------|
| `graphs/supervisor.py` | 根图：分类 → 会话/澄清/领域决策扇出 → 缺参追问 → 汇合 → 合规出口；含响应投影 |
| `graphs/llm_supervisor.py` | LLM Supervisor 决策节点：以 `create_agent` + `transfer_to_<domain>` handoff 工具选定领域，统一构造根图 `Send` |
...
- **领域决策由 LLM Supervisor 承担**：`supervisor` 节点用 `create_agent` 通过
  `transfer_to_<domain>` handoff 工具选定业务领域（同一轮可并行 handoff 多个），
  再由节点统一构造根图 `Send`。分类器给出的候选领域是**硬上界**：LLM 只能在候选内
  取舍，越界的 handoff 被拒绝并丢弃；LLM 未选出任何有效领域时回退候选全集。
- **单领域与多领域是同一条路径**：`supervisor` 为每个被选中的业务领域铺开一条
  自包含任务描述，`Send` 并行扇出；单领域只是扇出数为 1。截止时间、停止语义、
  崩溃恢复对两种场景因此完全一致。
...
- **每轮派生状态收在单个 `run` 键**，由 `classify` 用 `Overwrite` 整键重置；
  ...
  结构上不可能。
- **紧急回退开关**：`ORCHESTRATION_SUPERVISOR_MODE=legacy` 可把领域决策切回确定性
  `scope_tasks`（`llm` 为默认）。这是**临时**急停开关，观察期结束后连同
  `scope_tasks` 路径一并移除。
...
- **跨领域扇出只由根图的 `Send` 表达**，不得再出现"节点内同步 invoke 一个计划子图"
  ...
  LLM Supervisor 的 `Send` 构造在 `graphs/llm_supervisor.py`，扇出目标仍是根图登记的
  `domain_worker`（架构测试同时守卫两处）。
- **LLM 只选域，不改写任务描述**：`supervisor` 的 handoff 描述仅在确定性改写退化时
  兜底；到达专家的任务描述以 `task_rewrite`（含实体保真校验）为准。
