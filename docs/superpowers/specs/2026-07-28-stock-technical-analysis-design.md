...
- `finance_agent/agents/supervisor.py` — 不新增意图，不修改分类逻辑
- `finance_agent/agents/data_fetch.py` — 不做修改，K线数据已通过 `get_stock_history` 获取
- `finance_agent/config.py` — 可新增 `technical` 温度配置项（可选）
- `finance_agent/core/shared_state.py` — 不做修改
