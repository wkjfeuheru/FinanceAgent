...
并把合并结果写入 ``evals/results/summary.json``。
"""
...
def main(argv: list[str] | None = None) -> int:
    from evals import (
        ...
        plan_fanout_benchmark as fanout,
    )
    ...
    fanout_res = _load("plan_fanout_benchmark")
    fault_res = _load("degradation_fault_injection")

    plan_4 = next(r for r in fanout_res["plan_fanout"] if r["domains"] == 4)
    noise = {
    ...
    summary = {
        ...
        "plan_fanout": {
            "four_domain_speedup": plan_4["speedup"],
            "four_domain_serial_s": plan_4["serial_median_s"],
            "four_domain_send_s": plan_4["send_median_s"],
            "dependency_order_preserved": fanout_res["dependency_dag"]["dependency_order_preserved"],
    ...
    pf = summary["plan_fanout"]
    lines.append(f"- 4 领域计划扇出加速比：**{pf['four_domain_speedup']:.2f}x**"
                 f"（{pf['four_domain_serial_s']:.3f}s → {pf['four_domain_send_s']:.3f}s）")
    lines.append(f"- 多标的取数加速比（3/6/10 标的）："
                 f"{', '.join(f'{v:.2f}x' for v in pf['fetch_speedups'].values())}")
