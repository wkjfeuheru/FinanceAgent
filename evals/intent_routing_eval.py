...
产出：stdout 的 Markdown 报告 + ``evals/results/intent_routing_eval.json``。
"""
...
def build_cases() -> list[Case]:
    ...
    m = "我的持仓怎么样"
    add(Case("s10", m, _payload(_intent("account_query", m, m, execution_mode="position_query")),
             ("account_portfolio",), "domain_react"))

    m = "我账户里还有多少钱"
    add(Case("s11", m, _payload(_intent("account_query", m, m, execution_mode="account_overview")),
             ("account_portfolio",), "domain_react"))

    m = "帮我买1000块110011"
    add(Case("s12", m, _payload(_intent("account_query", m, m, execution_mode="trade_guidance")),
             ("account_portfolio",), "domain_react"))
    ...
    ), ("stock_research", "product_research"), "plan_execute"))

    m = "今天大盘怎么样，顺便帮我看看我的持仓"
    add(Case("p2", m, _payload(
        _intent("market_insight", "今天大盘怎么样", "今天大盘怎么样", execution_mode="market_overview"),
        _intent("account_query", "看看我的持仓", "看看我的持仓", execution_mode="position_query"),
    ), ("market_insight", "account_portfolio"), "plan_execute"))
    ...
    ), ("stock_research", "market_insight"), "plan_execute"))

    m = "帮我看看大盘和我的账户资金"
    add(Case("p4", m, _payload(
        _intent("market_insight", "看看大盘", "看看大盘", execution_mode="market_overview"),
        _intent("account_query", "我的账户资金", "我的账户资金", execution_mode="account_overview"),
    ), ("market_insight", "account_portfolio"), "plan_execute"))
    ...
    ), ("stock_research", "market_insight", "product_research"), "plan_execute"))
    ...
    ), ("market_insight",), "domain_react", tag="noise:mode_as_intent",
    ...
    ), ("stock_research",), "domain_react", tag="noise:mode_as_intent",
    ...
    ), ("account_portfolio",), "domain_react", tag="noise:mode_as_intent",
    ...
    ), ("stock_research",), "domain_react", tag="noise:unknown_sibling",
    ...
    ), ("stock_research", "market_insight"), "plan_execute", tag="noise:bad_mode_sibling",
    ...
    ), ("stock_research",), "domain_react", tag="noise:bad_evidence_sibling",
    ...
    add(Case("n7", m, _payload(
        ...
        _intent("product_analysis", "那个基金也看看", "那个基金也看看", confidence=0.7,
                execution_mode="product_analysis"),
    ), ("stock_research",), "domain_react", tag="noise:low_conf_sibling",
...
def strict_route(message: str, raw: dict) -> tuple[tuple[str, ...], str]:
    ...
    for item in intents:
        ...
        if mode not in _EXECUTION_MODES[intent]:  # 非法模式即失败
            return (FAIL,), "clarify"
    ...
    mode = "domain_react" if len(ordered) == 1 else "plan_execute"
    return ordered, mode
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json", default="evals/results/intent_routing_eval.json")
