"""库表漂移诊断：对比「DDL 声明的期望结构」与「线上实际结构」。
    ...
    做法：把 `migrations/` 里的建表脚本应用到一个一次性探针库，作为**权威期望结构**
...
用法：
    python -m finance_agent.cli.diagnose_schema_drift               # 临时建库对比，结束后自动删除探针库
    python -m finance_agent.cli.diagnose_schema_drift --keep-probe  # 保留探针库以便人工检查
...
from finance_agent.infrastructure.settings import _postgres_dsn
...
# 与部署 migrate 和 FAQ 懒加载共用同一份结构清单；007 是种子数据，故跳过。
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="诊断 PostgreSQL 结构漂移")
    parser.add_argument(
        "--keep-probe", action="store_true",
        help="完成后保留 advisor_schema_probe 探针库",
    )
