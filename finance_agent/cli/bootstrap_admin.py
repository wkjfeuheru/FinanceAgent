...
本脚本把角色落进 ``finance.users.is_admin``（``migrations/013_admin_console.sql``），
使"谁是管理员"成为可持久、可审计的数据。

用法：

    # 创建（或提升）管理员 admin，并确保其他人都不是管理员
    python -m finance_agent.cli.bootstrap_admin --username admin --password 'YourPassw0rd'
...
def main(argv: list[str] | None = None) -> int:
    ...
    from finance_agent.infrastructure.persistence.postgres.auth_store import _validate_password
