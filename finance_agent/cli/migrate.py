...
``migrations/001``–``014`` 已是幂等 DDL 事实源。本模块是部署入口，失败显式退出。

用法::

    python -m finance_agent.cli.migrate
    python -m finance_agent.cli.migrate --seed
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m finance_agent.cli.migrate",
        description="幂等应用 migrations/ 结构脚本（含 FAQ / pgvector）；可选种子数据。",
    )
    parser.add_argument(
        "--seed",
        action="store_true",
        help="同时应用 migrations/007_product_seed.sql（ON CONFLICT 幂等）",
    )
    return parser
...
def apply_from_config(*, include_seed: bool) -> None:
    ...
    from finance_agent.infrastructure.persistence.postgres.schema import apply_postgres_schema
