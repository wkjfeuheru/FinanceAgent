...
- **写入只经此处**：Agent 对话领域只能读账户与持仓，下单与充值必须走本路由，
  与"投顾不代客操作"的既有取向一致。
- 所有响应标注"模拟交易"，不构成投资建议。
- **阻塞调用不进事件循环**：本模块 handler 一律是同步 ``def``，由 Starlette 放进线程池
  执行。``get_service()`` 背后是同步 psycopg 调用，写成 ``async def`` 会在事件循环里
  直接执行它们 —— 一次慢查询就足以冻住整个 API（连 ``/api/health`` 与新建连接一起）。
"""
...
from finance_agent.api.schemas.portfolio import (
    DepositRequest,
    DepositResponse,
    LiquidationRequest,
    LiquidationResponse,
...
from finance_agent.api.dependencies import _require_customer_id
from finance_agent.domains.portfolio.contracts import ProductView
from finance_agent.domains.portfolio.errors import (
...
def list_products(http_request: Request, product_type: str = "fund") -> ProductShelfResponse:
...
def get_product(http_request: Request, code: str) -> ProductView:
    """单个商品详情（含费率与最新净值）。"""
    _require_customer_id(http_request)
...
@router.get("/account")
def get_account(http_request: Request) -> dict[str, Any]:
...
def list_positions(http_request: Request) -> PositionListResponse:
...
def deposit(http_request: Request, payload: DepositRequest) -> DepositResponse:
...
def create_order(http_request: Request, payload: OrderRequest) -> TradeResponse:
...
def liquidate(http_request: Request, payload: LiquidationRequest) -> LiquidationResponse:
...
def list_orders(http_request: Request, limit: int = 100) -> OrderListResponse:
    """成交记录（含费用与已实现盈亏）。"""
    customer_id = _require_customer_id(http_request)
...
def list_transactions(http_request: Request, limit: int = 100) -> TransactionListResponse:
    """资金流水。"""
    customer_id = _require_customer_id(http_request)
