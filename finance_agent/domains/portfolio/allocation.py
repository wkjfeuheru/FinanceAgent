...
``statistics``，风格与 ``domains/research/technical.py`` 一致。账户领域用本模块
...
现有合规出口（``safety/output_policy.py`` + ``orchestration/graphs/compliance.py``）。
"""
...
def reference_bands(risk_preference: Any) -> tuple[str, dict[str, tuple[float, float]] | None]:
    ...
    """
    from finance_agent.domains.products.rules import normalize_profile_risk
...
def _default_risk_free() -> float:
    from finance_agent.domains.portfolio import settings as config
...
def _tier_midpoints(bands: Mapping[str, tuple[float, float]]) -> dict[str, float]:
    """把各档参考区间取中点作为该档目标占比（未归一）。"""
    return {tier: (low + high) / 2.0 for tier, (low, high) in bands.items()}
...
def optimization_plan(
    ...
    *,
    ...
    """给出配置优化参考：档位偏离、持仓级目标与优化前后指标对比。

    ``items`` 应包含**全部有价持仓**（``code``/``name``/``tier``/``weight``/
    ``volatility``/``return_1y``/``max_drawdown``；``volatility`` 可为空）。
    目标权重口径：

    - 提供 ``bands``（用户风险偏好对应的三档参考区间）时，按各档区间**中点**
      定档位目标；档内按 ``1/σ`` 分配（该档若存在缺波动率的持仓，则**整档退化
      为等权**，绝不把缺数据的持仓算成 0 权重）；
    - 未提供 ``bands`` 时按逆波动率口径分配；若存在缺波动率的持仓则**退化为
      等权**（与风险平价在对角近似下等价）；
    - 目标权重统一过 ``constrained_weights`` 投影到 ``[weight_min, weight_max]``。

    关键设计：优化对象是**全部有价持仓**而非"仅有波动率的子集"。否则当只有 1 只
    具备波动率时，目标会被归一成 100%、方向恒为"维持"，并掩盖真实的档位失衡
    （例如"全部持仓都是高风险"却显示为达标）。

    ``coverage`` 如实标注指标测算覆盖范围（缺波动率的持仓无法计算风险指标），
    ``tier_gaps`` 给出三档相对参考区间的偏离（含"当前无对应档位标的"），这是
    "全部集中于单一风险档位"场景下真正可操作的策略建议。
    """
    rows = [dict(item) for item in items if _finite(item.get("weight")) is not None]
    total = len(rows)
    vols = [_finite(item.get("volatility")) for item in rows]
    uncovered = [
        str(rows[index].get("code") or "")
        for index in range(total)
        if vols[index] is None or vols[index] <= 0
    ]
    coverage = {"covered": total - len(uncovered), "total": total, "uncovered": uncovered}
    if not rows:
        return {
            "basis": "none", "coverage": coverage, "targets": [],
            "before": {}, "after": {}, "tier_targets": {}, "tier_gaps": [],
            "unrealizable_tiers": [], "notes": [],
        }

    codes = [str(item.get("code") or "") for item in rows]
    names = [str(item.get("name") or "") for item in rows]
    tiers = [str(item.get("tier") or "") for item in rows]
    current = normalize_weights([item.get("weight") for item in rows])
    min_w = max(0.0, float(weight_min))
    max_w = min(1.0, float(weight_max))

    def _within(indexes: Sequence[int]) -> list[float]:
        """组内分配：全部具备正波动率时按 1/σ，否则退化为等权。"""
        values = [vols[index] for index in indexes]
        if values and all(value is not None and value > 0 for value in values):
            return normalize_weights([1.0 / value for value in values])
        return equal_weight(len(indexes))

    present = sorted({tier for tier in tiers})
    if bands:
        midpoints = {
            tier: value for tier, value in _tier_midpoints(bands).items() if value > 0
        }
        total_mid = sum(midpoints.values())
        band_abs = (
            {tier: value / total_mid for tier, value in midpoints.items()}
            if total_mid > 0 else {}
        )
        unrealizable = sorted(tier for tier in band_abs if tier not in present)
        shares = {tier: band_abs.get(tier, 0.0) for tier in present}
        share_total = sum(shares.values())
        if share_total <= 0:
            # 现有档位在参考模型中目标为 0（如 R1 的高风险上限为 0）：退回等权。
            shares = {tier: 1.0 for tier in present}
            share_total = float(len(present))
        target = [0.0] * total
        for tier in present:
            indexes = [index for index, value in enumerate(tiers) if value == tier]
            within = _within(indexes)
            share = shares[tier] / share_total
            for position, index in enumerate(indexes):
                target[index] = share * within[position]
        basis = "risk_band"
    else:
        target = _within(list(range(total)))
        band_abs = {}
        unrealizable = []
        basis = "inverse_volatility"

    target = constrained_weights(target, min_w, max_w)

    def _direction(delta: float, threshold: float = 0.02) -> str:
        if delta > threshold:
            return "提高"
        if delta < -threshold:
            return "降低"
    ...
    for index, code in enumerate(codes):
        delta = round(target[index] - current[index], 6)
        targets.append({
            "product_code": code,
            "product_name": names[index],
            "tier": tiers[index],
            "volatility": vols[index],
            "current": round(current[index], 6),
            "target": round(target[index], 6),
            "delta": delta,
            "direction": _direction(delta),
        })
    targets.sort(key=lambda row: -abs(row["delta"]))

    # 档位层偏离（含"当前无对应标的"）：单一档位占满时的唯一可操作方向在这里。
    tier_gaps: list[dict[str, Any]] = []
    if bands:
        tier_now: dict[str, float] = {}
        for index, tier in enumerate(tiers):
            tier_now[tier] = tier_now.get(tier, 0.0) + current[index]
        for tier in _TIER_KEYS:
            band = bands.get(tier)
            if band is None:
                continue
            low, high = band
            now = tier_now.get(tier, 0.0)
    ...
    for index, tier in enumerate(tiers):
        tier_targets[tier] = round(tier_targets.get(tier, 0.0) + target[index], 6)

    # 前后指标只在**可测算子集**（有波动率）上计算：缺波动率的持仓无法给出风险
    # 指标，如实标注覆盖范围而不臆造。权重在该子集内归一，保证可比。
    covered = [index for index in range(total) if vols[index] is not None and vols[index] > 0]
    if covered:
        before_weights = normalize_weights([current[index] for index in covered])
        after_weights = normalize_weights([target[index] for index in covered])
        c_vols = [vols[index] for index in covered]
        c_returns = [_finite(rows[index].get("return_1y")) for index in covered]
        c_drawdowns = [_finite(rows[index].get("max_drawdown")) for index in covered]
        before = {
            "volatility": _round_or_none(portfolio_volatility(before_weights, c_vols)),
            "expected_return": _round_or_none(portfolio_return(before_weights, c_returns)),
            "max_drawdown": _round_or_none(portfolio_max_drawdown(before_weights, c_drawdowns)),
        }
        after = {
            "volatility": _round_or_none(portfolio_volatility(after_weights, c_vols)),
            "expected_return": _round_or_none(portfolio_return(after_weights, c_returns)),
            "max_drawdown": _round_or_none(portfolio_max_drawdown(after_weights, c_drawdowns)),
        }
    else:
        before, after = {}, {}
    ...
    notes: list[str] = []
    if basis == "risk_band":
        notes.append("目标占比按风险偏好的三档参考区间中点定档、档内按波动率倒数分配（缺波动率则等权）。")
    else:
        notes.append("目标占比按逆波动率口径分配（对角近似下与风险平价一致；缺波动率则等权）。")
    notes.append("方向为测算参考，不是交易指令；优化前后指标为对角近似口径，未考虑相关性。")
    ...
    return {
        ...
        "tier_gaps": tier_gaps,
        "unrealizable_tiers": unrealizable,
        "notes": notes,
    }
...
def review_portfolio(
    ...
    *,
    ...
    optimization = optimization_plan(
        [
            {
                "code": _code_of(item), "name": str(item.get("product_name") or ""),
                "tier": _tier_of(item.get("risk_level")), "weight": weight,
...
def _tier_of(risk_level: Any) -> str:
    from finance_agent.domains.products.rules import normalize_risk
