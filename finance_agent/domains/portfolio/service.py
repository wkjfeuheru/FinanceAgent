...
from finance_agent.domains.portfolio.settings import (
    PORTFOLIO_MAX_DEPOSIT_AMOUNT,
    PORTFOLIO_MIN_ORDER_AMOUNT,
)
from finance_agent.domains.portfolio.contracts import (
    AccountSnapshot,
    LiquidationResult,
    OrderView,
    PositionView,
...
from finance_agent.domains.portfolio.fees import (
    redemption,
    round_money,
    round_shares,
...
from finance_agent.domains.portfolio.pricing import (
    NavSource,
    require_nav,
)
...
class PortfolioDeps:
    ...
    def build_store(self) -> Any:
        if self.store is not None:
            return self.store
        raise RuntimeError("PortfolioDeps.store must be provided by the composition root")

    def build_library(self) -> Any:
        if self.library is not None:
            return self.library
        raise RuntimeError("PortfolioDeps.library must be provided by the composition root")

    def build_nav_source(self) -> NavSource:
        if self.nav_source is not None:
            return self.nav_source
        raise RuntimeError("PortfolioDeps.nav_source must be provided by the composition root")
...
class PortfolioService:
    ...
    def _to_product_view(self, code: str, product: dict[str, Any], nav_quote: Any) -> ProductView:
        from finance_agent.domains.portfolio.pricing import product_fee_rates
    ...
    def _replay_deposit_conflict(
        ...
        """并发同一幂等键时，把 unique 冲突转为首次充值流水的重放。"""
