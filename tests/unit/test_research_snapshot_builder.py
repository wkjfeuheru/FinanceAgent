...
def test_equivalent_fallback_is_visible_in_fact_evidence():
    ...
    assert snapshot.securities[0].quote.fallback_from == "fuyao_mcp"
    assert any(fact.payload.get("fallback_from") == "fuyao_mcp" for fact in facts)
