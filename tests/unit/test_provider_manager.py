...
def test_unsupported_capability_is_not_recorded_as_degradation():
    ...
    """
    a = FakeProvider("akshare", unsupported=["get_daily_basic"])
    b = FakeProvider("fuyao_mcp")
    manager = _manager({"akshare": a, "fuyao_mcp": b}, ["akshare", "fuyao_mcp"])

    manager.get_daily_basic("600519")

    assert manager.last_metadata["source"] == "fuyao_mcp"
...
def test_unavailable_provider_skipped():
    ...
    down = DownProvider("fuyao_mcp")
    up = FakeProvider("akshare")
    manager = _manager({"fuyao_mcp": down, "akshare": up}, ["fuyao_mcp", "akshare"])
...
def test_manager_singleton_is_thread_safe(monkeypatch):
    ...
    from finance_agent.infrastructure.market_data import provider_manager as pm
