...
from finance_agent.orchestration.routing.intent import (
...
def test_legacy_sub_intent_or_execution_mode_field_recovers_intent():
    """旧协议的 sub_intent/execution_mode 字段仍被读取，但只用于还原误填的 intent。

    历史 checkpoint / 旧模型输出可能把模式名放进这两个字段而不是 intent。
    """
    from finance_agent.orchestration.routing.intent import normalize_intent_item
...
# ``test_prompt_forbids_mode_values_in_intent_field`` 已删除：分类提示词不再向模型
# 索要子意图/模式，也不再就此约束 intent 字段取值。模式名误填仍由
# ``_MODE_TO_INTENT`` 容错还原（见上）。


def test_prompt_confidence_threshold_is_not_hardcoded_twice():
    """提示词里的置信度阈值必须取自常量，避免文案与过滤规则双写漂移。

    子意图阈值常量 ``_SUB_INTENT_CONFIDENCE_THRESHOLD`` 已随子意图体系删除，
    现在只剩意图阈值一个常量，其文案在提示词里只允许出现一次（由常量插值而来）。
    """
    from finance_agent.orchestration.routing.intent import (
