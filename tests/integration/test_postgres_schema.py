...
def test_schema_files_are_numbered_without_gaps():
    ...
    assert numbers == list(range(numbers[0], numbers[-1] + 1)), (
        f"migrations/ 脚本编号存在缺号：{numbers}"
    )


def test_schema_apply_order_covers_sql_except_seed():
    """统一清单覆盖可执行迁移；005 已退役，仅为历史脚本留档。"""
    sql_files = {
        path.name for path in SQL_DIR.glob("0*.sql")
        if path.name != "005_theme_registry.sql"
    }
    ...
    assert "005_theme_registry.sql" not in applied
    numbers = sorted(
        int(name[:3]) for name in (*SCHEMA_APPLY_ORDER, *FAQ_SCHEMA_APPLY_ORDER)
    )
    assert numbers == [1, 2, 3, 4, 6, 8, 9, 10, 11, 12, 13, 14]
...
def test_theme_removal_migration_preserves_research_audit_tables():
    for table in (
        "theme_evidence", "theme_reviews", "theme_memberships", "themes",
        "research_feature_snapshots",
    ):
        assert f"DROP TABLE IF EXISTS finance.{table}" in REMOVE_THEME_FEATURES_SCHEMA_SQL
    assert "DROP TABLE IF EXISTS finance.research_runs" not in REMOVE_THEME_FEATURES_SCHEMA_SQL
    assert "DROP TABLE IF EXISTS finance.research_results" not in REMOVE_THEME_FEATURES_SCHEMA_SQL
