...
from finance_agent.research.replay import (
    EVIDENCE_INCOMPLETE,
    MISMATCHED,
    RULES_UNAVAILABLE,
    replay_research_run,
...
def _run(codes=("600519",)):
    """跑一次真实流水线，并把它投影成审计存储的形状。"""
    request = _request(codes)
    pipeline = ResearchPipeline(
        snapshot_builder=SnapshotBuilder(Gateway(codes)),
        rule_engine=RuleEngine.default(),
    )
    result, facts = pipeline.analyze_with_facts(request, user_profile={})
    stored_results = [
        {
            "stock_code": code,
...
def test_replay_reproduces_insufficient_data_conclusion():
    ...
    pipeline = ResearchPipeline(
        snapshot_builder=SnapshotBuilder(FailingGateway()),
        rule_engine=RuleEngine.default(),
    )
    result, facts = pipeline.analyze_with_facts(request, user_profile={})
    record = {
