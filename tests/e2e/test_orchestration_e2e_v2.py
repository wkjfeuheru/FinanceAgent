"""端到端：复合请求经 ``Send`` 扇出与汇合后到合规出口，不依赖外部服务。

（计划层 ``plan_execute`` 已删除：单/多领域共用 ``domain_workflow`` 一条路径，
多领域只是 ``Send`` 扇出数大于 1，本文件断言随之从"经 Plan-and-Execute"改为
"经扇出 + 汇合"。断言读取的是 ``handle_message`` 的响应投影，键名未变。）
"""
...
def test_composite_request_fans_out_then_compliance(monkeypatch):
