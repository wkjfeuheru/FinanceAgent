...
# 允许 `python -m tests.e2e.e2e_verify`：未 editable 安装时显式补上仓库根目录，
# 确保可 import finance_agent（否则会
...
def assert_chat(token, message, *, domain=None, needles=(), forbidden=(), degraded=None, mode=None):
    ...
    if mode == "fanout":
        check(
            f"chat '{message}' fans out to both domains",
...
def main() -> int:
    ...
    try:
        import redis
        from finance_agent.infrastructure import settings as _config
