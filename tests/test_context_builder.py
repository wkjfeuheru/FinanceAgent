...
def test_synthesis_context_contains_status_and_warnings_without_raw_history():
    task = Task(
        task_id="task-1", intent=IntentKind.STOCK_ANALYSIS,
        expert_name="stock_analysis", requirement="分析", execution_mode="stock_analysis",
    )
    state = {"user_message": "分析", "memory_context": "原始历史"}
    result = {
        "task_id": "task-1", "intent": "stock_analysis", "expert_name": "stock_analysis",
