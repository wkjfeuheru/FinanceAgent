...
def test_daily_records_are_sorted_ascending_across_vendor_date_formats():
    """Tushare 返回 YYYYMMDD 且可能倒序，必须统一为升序 ISO 日期。"""
    rows = normalize_daily_records([
...
def test_trade_calendar_fields_are_unified_across_vendors():
    ...
    from finance_agent.data.normalization import normalize_trade_cal_records

    tushare = normalize_trade_cal_records({
    ...
    assert tushare["data"][0]["cal_date"] == "2026-09-11"
    assert tushare["data"][0]["is_open"] == 1
...
def akshare_only_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "finance_agent.orchestrator.tools.stockdata.get_provider_manager",
        lambda: manager,
    )
    return manager
