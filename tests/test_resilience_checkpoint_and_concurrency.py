...
这三项对应 rules/langgraph-guides/fault-tolerance.md 之外的可用性缺口：
...
def test_root_graph_persists_state_by_thread_id():
    ...
    assert snapshot.values.get("final_response") == result["final_response"]
...
def test_handle_message_passes_thread_id_in_config(monkeypatch):
    ...
    from finance_agent.config import ORCHESTRATION_GRAPH_STEPS
...
def test_trusted_content_does_not_leak_across_turns():
    ...
    assert first["trusted_content"] is True  # 本轮确实是受信内容

    second = graph.invoke({"user_message": "分析贵州茅台", "run_id": "r2"}, config)

    # 风险话术必须被处理掉（改写或拦截），绝不能因残留的 trusted_content 放行。
    assert second["trusted_content"] is False
    assert "稳赚不赔" not in second["final_response"]
    assert second["compliance"]["action"] in {"rewritten", "blocked"}
...
def test_warnings_do_not_leak_across_turns():
    ...
    graph.invoke({"user_message": "你好", "run_id": "w1", "warnings": ["stale"]}, config)
    second = graph.invoke({"user_message": "你好", "run_id": "w2"}, config)

    assert "stale" not in second["warnings"]
