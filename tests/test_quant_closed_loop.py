...
def test_stock_domain_submits_with_full_context_and_reports_pending_jobs():
    ...
    context = DomainTaskContext(
        task=PlanTask(
            task_id="single:run-1:stock_research", domain=BusinessDomain.STOCK_RESEARCH,
            goal="分析贵州茅台", instruction="分析贵州茅台", expected_output="domain_outcome",
        ),
        thread_id="v1:CUST1:conv-1", customer_id="CUST1", conversation_id="conv-1",
        user_message="分析贵州茅台", run_id="run-1",
    )
    stock_data = {
        "600519": {"history": {"data": [
    ...
    _, pending, pending_jobs = compute_technical_via_gateway(
        deps, stock_data, ["600519"], context=context,
    )

    assert pending is True
    # 四项元数据必须全部落到 submit：缺失则 job 引用无法落库/授权查询。
    submitted = gateway.submits[0]
    assert submitted["customer_id"] == "CUST1"
    assert submitted["thread_id"] == "v1:CUST1:conv-1"
    assert submitted["run_id"] == "run-1"
    assert submitted["task_id"] == "single:run-1:stock_research"
    assert pending_jobs and pending_jobs[0][0] == "600519"
