...
运行：python tools/verify_portfolio_live.py
"""
...
from finance_agent.data.postgres_schema import PORTFOLIO_SCHEMA_SQL
...
def main() -> int:
    ...
    from finance_agent.portfolio.service import (
        PortfolioDeps,
        PortfolioService,
    )
    ...
    service = PortfolioService(PortfolioDeps(
        store=get_portfolio_store(), library=get_product_library(),
    ))
